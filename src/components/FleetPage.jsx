...
export function FleetPage({ openVehicle }) {
  ...
  const [driveFilter, setDriveFilter] = useState({ fwd: true, rwd: true, awd: true });
  ...
  const filteredFleet = fleet.filter(v => {
    ...
    return matchesTrans && matchesSeats && matchesDrive;
  });
    ...
    <div className="view container section" style={{ paddingTop: '32px' }}>
      ...
      <div className="cat-layout">
        ...
        <aside className="filters">
          ...
          {/* Meghajtás szűrő */}
          <div className="filter-group">
            <h5 style={{ margin: '0 0 10px 0', fontSize: '14px', fontWeight: 700 }}>Meghajtás</h5>
            <label className="check">
              <input
                type="checkbox"
                checked={driveFilter.fwd}
                onChange={(e) => setDriveFilter(prev => ({ ...prev, fwd: e.target.checked }))}
              />
              <span style={{ display: 'inline-flex', alignItems: 'center', gap: '6px' }}>
                <Icon name="arrow-up" size={14} style={{ color: 'var(--go-600)' }} /> Elsőkerék-hajtás
              </span>
            </label>
            <label className="check">
              <input
                type="checkbox"
                checked={driveFilter.rwd}
                onChange={(e) => setDriveFilter(prev => ({ ...prev, rwd: e.target.checked }))}
              />
              <span style={{ display: 'inline-flex', alignItems: 'center', gap: '6px' }}>
                <Icon name="rotate-cw" size={14} style={{ color: 'var(--amber-600)' }} /> Hátsókerék-hajtás
              </span>
            </label>
          </div>
          ...
          {filteredFleet.length > 0 ? (
            ...
            <div style={{ textAlign: 'center', paddingBlock: '60px', background: 'var(--card)', borderRadius: 'var(--r-lg)', border: '1px dashed var(--line)' }}>
              ...
              <Button variant="ghost" style={{ marginTop: '12px' }} onClick={() => {
                ...
                setDriveFilter({ fwd: true, rwd: true, awd: true });
              }}>
